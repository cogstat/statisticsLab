...
For this chapter, we will use the data from  (Mehr et al., 2016, see: https://journals.sagepub.com/doi/10.1177/0956797615626691), a study of how music may convey important social information to infants.
...
There is a column called “Baseline_Proportion_Gaze_to_Singer”, which contains values that show the proportion of time that the infant looked at the person who would later sing the familiar song to them. If the average of these proportions is 0.5 across the infants, that would provide some evidence that the infants were not biased at the beginning of the experiment. However, if the infants on average had a bias towards one of the people displayed, then the average proportion of the looking time should be different than 0.5.

In this task, the null hypothesis is that the baseline mean does not differ from the 0.5 mean, which means that the time spent looking at both people is about equal. Using a one-sample t-test, we can test the hypothesis that our sample mean for the “Baseline_Proportion_Gaze_to_Singer” was not different from 0.5.

First, open the dataset “MehrSongSpelke2016_cut” in CogStat. To check whether the baseline is different or not from 0.5, we will run a one-sample t-test. Choose “Analysis” and then “Explore variable”. After the dialogue appears, follow these steps: choose the variable “Baseline_Proportion_Gaze_to_Singer” as “Selected variable(s)”, and set the “Central tendency test value” to 0.5 after that, press “OK”. As mentioned before, the null hypothesis is that the mean of the baseline value will not be significantly different from 0.5, which we set earlier.

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ch6/6.4expvarwindow.png')
```
...
```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ch6/6.4expvarraw.png')
```
...
```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ch6/6.4descr.png')
```
...
```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ch6/6.4poppropSW.png')
```
...
```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ch6/6.4hyptest.png')
```

The result of the one-sample t-test is t(31)=0.67, p=0.505. Take a look at the values. The value 31 in the results is the degrees of freedom (df=n-1). 0.67 is the t-statistic value, which is the difference between the observed mean (here the mean of the baseline) and the expected mean (here 0.5) divided by the standard error (which is the standard deviation/√elements of the sample). As p>0.05 in this case (p=0.505), the mean of the time spent looking is not significantly different from 0.5, so we cannot reject the null hypothesis based on the information given.

### One-sample t-test in CogStat- Test phase
...
```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ch6/6.5poppropSW.png')
```

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ch6/6.5hyptest.png')
```
...
### Paired Samples t-test in CogStat
...
1. There might be no significant difference found between the baseline and test phase in terms of gaze towards the singer.

2. We might find differences between the two settings, negative as well as positive changes also can be found.

3. We might find that the gaze proportion from the baseline to the test phase decreases or increases.
...
```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ch6/6.7comparerepeatedmeasures_icon.png')
```

or

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ch6/6.7comparerepeatedmeasures_menu.png')
```

Select the variables “Test_Proportion_Gaze_to_Singer” and Baseline_Proportion_Gaze_to_Singer” for the analysis, and then press"OK ". 

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ch6/6.7comparerepeatedmeasures_window.png')
```
...
```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ch6/6.7comparerepeatedmeasures_info.png')
```

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ch6/6.7comparerepeatedmeasures_raw.png')
```
...
```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ch6/6.7comparerepeatedmeasures_sampleprop.png')
```

In sample properties boxplots are also provided, to show the representation of the data used in the analysis. 

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ch6/6.7comparerepeatedmeasures_boxplot.png')
```
...
```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ch6/6.7comparerepeatedmeasures_hyptest.png')
```
...
### Relationship between one-sample and paired-samples t-test
...
```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ch6/6.8expvar_window.png')
```
...
```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ch6/6.8expvar_hyptest.png')
```
