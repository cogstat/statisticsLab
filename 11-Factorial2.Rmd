...
Hit “Factors…” in the dialogue.

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ch11/11.3CRMVG_window.png')
```

Choose “Add factor”.

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ch11/11.3CRMVG_factorswindow.png')
```

Here a within-subject variable will be used as a factor, with 2 levels. Name the factor as test_type and set the number of levels to two. Finally, hit “OK”.

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ch11/11.3CRMVG_setfactor.png')
```

Test_type 1 will  be “apercent” and Test_type 2 will be “vpercent”. Condition is a between-subject variable (one person only belongs to one condition), place it into the Group field. After that, hit “OK”.

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ch11/11.3CRMVG_setvariables.png')
```
...
```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ch11/11.3CRMVG_raw.png')
```
...
```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ch11/11.3CRMVG_descriptives.png')
```

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ch11/11.3CRMVG_plot.png')
```
...
```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ch11/11.3CRMVG_CI_plot.png')
```

The last section of Population parameters is the hypothesis test, which shows the result of the mixed ANOVA. From results we see that the main effect of condition is not significant (F(1,295)=2.26, p=0.134, the main effect of test type turned out to be significant (F(1,295)=100.23, p< 0.001, and finally the interaction between condition and test type is also significant (F(1,295)=13.79, p<0.001.

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/ch11/11.3CRMVG_hyptest.png')
```

You might notice that in this analysis there were not any assumptions tested or post-hoc tests run as before in previous ANOVA analyses, it is because these functions are not implemented yet, in the current CogStat package. (CogStat2.5_rc)
...
### REFERENCES
  ...
  Roediger III, H. L., & Karpicke, J. D. (2006). Test-Enhanced Learning: Taking Memory Tests Improves Long-Term Retention on JSTOR. Psychological Science, 249. https://doi.org/40064526

  Mehr, S. A., Song, L. A., & Spelke, E. S. (2016). For 5-Month-Old Infants, Melodies Are Social. Psychological Science. https://doi.org/10.1177/0956797615626691

  Kessler, H., Schmidt, A. C., James, E. L., Blackwell, S. E., von Rauchhaupt, M., Harren, K., ... & Holmes, E. A. (2020). Visuospatial computer game play after memory reminder delivered three days after a traumatic film reduces the number of intrusive memories of the experimental trauma. Journal of Behavior Therapy and Experimental Psychiatry, 67, 101454.
